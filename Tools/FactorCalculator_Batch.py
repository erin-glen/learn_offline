...
if __name__ == "__main__":
    ...
    inputConfig = dict(
        ...
        treecanopy_1=os.path.join(alternateDataFolder, "Salem", "TC_2009.tif"),
        treecanopy_2=os.path.join(alternateDataFolder, "Salem", "TC_2018.tif"),
    ...
    # aoi list can pull from separate prep script (1) or be hardcoded (2)
    aoi_list = datalist
...
#code to return only factors from forest table (and convert to CO2)
    ...
    forestAge['Annual_Emissions_Fire_CO2'] = ((forestAge['fire_HA'] * forestAge['Fire Emissions Factor']) * (44/12) / (int(year2) - int(year1)))

    forestAge['Annual_Emissions_Harvest_CO2'] = ((forestAge['harvest_HA'] * forestAge['Harvest Emissions Factor']) * (44 / 12) / (int(year2) - int(year1)))

    forestAge['Annual_Emissions_Insect_CO2'] = ((forestAge['insect_damage_HA'] * forestAge['Insect Emissions Factor']) * (44 / 12)/ (int(year2) - int(year1)))

    forestAge['Annual_Removals_CO2'] = ((forestAge['undisturbed_HA'] * forestAge['Forests Remaining Forest Removal Factor']) * (44 / 12))

    forestAge['fire_product'] = forestAge['fire_HA'] * forestAge['Fire Emissions Factor']
    forestAge['insect_product'] = forestAge['insect_damage_HA'] * forestAge['Insect Emissions Factor']
    forestAge['harvest_product'] = forestAge['harvest_HA'] * forestAge['Harvest Emissions Factor']
    forestAge['undisturbed_product'] = forestAge['undisturbed_HA'] * forestAge['Forests Remaining Forest Removal Factor']

    forestAge['AOI'] = str(aoi_name)
